<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="体育馆进入时间" prop="tiyuguanjinrushijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.tiyuguanjinrushijian" 
                type="datetime"
                placeholder="体育馆进入时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tiyuguanjinrushijian" label="体育馆进入时间" prop="tiyuguanjinrushijian">
              <el-input v-model="ruleForm.tiyuguanjinrushijian" 
                placeholder="体育馆进入时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.tiyuguanjinrushijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='tiyuguanjinrushijian'){
            this.ruleForm.tiyuguanjinrushijian = obj[o];
	    this.ro.tiyuguanjinrushijian = true;
            continue;
          }
          if(o=='tiwen'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jinrudengji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jinrudengjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
